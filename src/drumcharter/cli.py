"""Command-line interface for DrumCharter.

Provides ``drumcharter generate`` for drum chart generation and
``drumcharter train`` for model fine-tuning subcommands.
"""
...
from drumcharter.branding import (
    print_branding,
    print_generated_chart,
    print_generation_summary,
)
from drumcharter.config import DEFAULT_CHARTER, config_exists, load_config, save_config
from drumcharter.device import DeviceError, VALID_TORCH_DEVICES, resolve_torch_device
from drumcharter.download import (
    DownloadError,
    DownloadSearchNoResults,
    download_audio_search,
)
from drumcharter.inference.fake import FakeTranscriber
from drumcharter.pipeline import STAGES, generate_drum_chart_folder
from drumcharter.postprocess import QUANTIZE_CHOICES
from drumcharter.separation import SeparationError
...
def _setup_logging(verbose: bool) -> None:
    ...
    logging.getLogger("drumcharter").setLevel(
...
def _download_search_audio(query: str, tmp_path: Path, *, verbose: bool) -> Path:
    """Download the first YouTube search result and present clean CLI errors."""
    with console.status(f'[bold green] Download audio: "{query}"...'):
        try:
            return download_audio_search(query, tmp_path, verbose=verbose)
        except DownloadSearchNoResults as e:
            console.print(f"[red]{e}[/red]")
            raise SystemExit(1) from e
        except DownloadError as e:
            console.print("[red]YouTube search/download failed.[/red]")
            if verbose:
                console.print(f"[red]{e}[/red]", soft_wrap=True)
            else:
                console.print(
                    "[yellow]Run again with [bold]--verbose[/bold] to show yt-dlp output.[/yellow]"
                )
            raise SystemExit(1) from e
...
def generate_cmd(
    ...
    if audio is None and song is None and artist is None:
        ...
        with ExitStack() as stack:
            if params["source_audio"] is None:
                ...
                tmp_path = Path(stack.enter_context(tempfile.TemporaryDirectory(prefix="drumcharter-yt-")))
                audio_file = _download_search_audio(query, tmp_path, verbose=verbose)
            else:
    ...
    with ExitStack() as stack:
        if audio is None:
            ...
            query = f"{artist} {song}"
            tmp_path = Path(stack.enter_context(tempfile.TemporaryDirectory(prefix="drumcharter-yt-")))
            audio = _download_search_audio(query, tmp_path, verbose=verbose)
...
@click.option("--cache-dir", type=click.Path(path_type=Path), default=None, help="Cache directory (default: ~/.cache/drumcharter/training)")
...
def prepare(
    ...
    import time
    from drumcharter.training.pipeline import PipelineConfig, run_pipeline

    if cache_dir is None:
        cache_dir = Path.home() / ".cache" / "drumcharter" / "training"
...
def frame(
    ...
    import logging
    from drumcharter.training.config import TrainConfig
    from drumcharter.training.train import run_training

    logging.basicConfig(level=logging.INFO, format="%(asctime)s %(levelname)s %(name)s: %(message)s", force=True)
    logging.getLogger("drumcharter").setLevel(logging.INFO)
...
def eval_frame(
    ...
    from drumcharter.training.dataset import _load_entries, create_splits
    from drumcharter.training.evaluate import CLASS_NAMES_8, evaluate, format_report
