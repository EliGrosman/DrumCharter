...
Stores and loads settings from ``~/.config/drumcharter/config.json``
...
_CONFIG_DIR = Path.home() / ".config" / "drumcharter"
_CONFIG_PATH = _CONFIG_DIR / "config.json"

_PACKAGE_ROOT = Path(__file__).resolve().parent.parent.parent
DEFAULT_CHARTER = "DrumCharter (AI)"
...
def load_config() -> dict:
    ...
    try:
        ...
        merged.update(data)
        if merged.get("charter") in ("", None, "DrumCharter"):
...
def config_exists() -> bool:
    ...
    Returns:
        True if ``~/.config/drumcharter/config.json`` exists.
    """
