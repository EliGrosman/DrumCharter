...
def _process_archive(archive_path: str, cache_dir: str, force: bool) -> list[dict]:
    from pathlib import Path
    from audiotochart.training.discovery import discover_songs
    from audiotochart.training.extract import extract_archive
    from audiotochart.training.rb_midi import has_pro_markers
    from audiotochart.training.spectrogram import process_single_song, song_hash

    archive = Path(archive_path)
    results = []
    tmp = tempfile.TemporaryDirectory(prefix="audiotochart-worker-")
    try:
...
def _process_song_direct(song_args: dict, cache_dir: str, force: bool) -> dict | None:
    from audiotochart.training.spectrogram import process_single_song
...
def run_pipeline(source_dirs: list[Path], config: PipelineConfig) -> PipelineResult:
    from audiotochart.training.discovery import discover_songs
    from audiotochart.training.extract import list_archives
    from audiotochart.training.rb_midi import has_pro_markers
    from audiotochart.training.spectrogram import song_hash
